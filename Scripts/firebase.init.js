...
import {collection,addDoc,getFirestore,getDocs,setDoc,doc,updateDoc,arrayUnion,getDoc,onSnapshot,query} from "https://www.gstatic.com/firebasejs/9.1.3/firebase-firestore.js";
...
document.getElementById('linkSignOut').addEventListener('click', function() {
  signOut(auth).then(() => {
    // Sign-out successful.
    console.log('logged out!')
  }).catch((error) => {
...
window.startChat= async function startChat(friendKey, friendName, friendPhoto) {
  ...
  querySnapshot.forEach(async (doc)=>{
    ...
    //refresh chat panel after clicking another friend to start chat
 ...
 async function loadChatMessages(chatKey, friendPhoto) {

  const snapshot = await query(collection(db, "chatMessages"));
  onSnapshot(snapshot, (querySnapshot) => {
    let messageDisplay = '';
    querySnapshot.forEach((doc)=>{
    if (doc.id === chatKey){
      let chat = doc.data();
      let arraySession = chat.session
      arraySession.forEach((item)=>{
      let dateTimeVar = item.dateTime.split(",")
      
      if (item.userID !== currentUserKey){
        messageDisplay += `<div class="row">
                              ...
                              <div class="col-6 col-sm-6 col-md-6">
                                  <p class="receive">
                                    ${item.msg}
                                    <span class="time float right" title="${dateTimeVar[0]}">${dateTimeVar[1]}</span>
                                  </p>
                              </div>
                          </div>`;
        }
...
async function sendMessage() {
  let chatMessage = {userID:currentUserKey, msg: document.getElementById('txtMessage').value, dateTime: new Date().toLocaleString()};
  
  const docRef = doc(db, 'chatMessages', chatKey);
  const docSnap = await getDoc(docRef);
  if (!docSnap.exists()) {await setDoc(docRef, {session: chatMessage}, { merge: true })}
  else {
...
.catch((error)=>{
  console.log("Message could not have been sent due to an error:"+ error)
})}}
...
async function loadChatList (){  
  ...
  querySnapshot.forEach(async (docFriendList)=>{
    let data = docFriendList.data();
    let friendKey = "";
  if(data.friendId === currentUserKey){friendKey = data.userId  } 
  else if (data.userId === currentUserKey) {friendKey = data.friendId};
  
  if (friendKey !== ""){
    const docRef = doc(db, 'users', friendKey);
    await getDoc(docRef).then((docFriendKey)=>{
      let user = docFriendKey.data();
      document.getElementById('listChat').innerHTML += `<li id="listFriend" class="list-group-item list-group-item-action" onclick = "startChat('${docFriendKey.id}', '${user.name}', '${user.photoURL}')">
                                                            <div class="row">
                                                                <div class="col-md-2">
